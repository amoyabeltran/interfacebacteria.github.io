...
from dashboard.funcionesYDatos import *
 ...
 ''')
   ...
   # Mostrar el gráfico según la selección
    if seleccion_secundaria == "Global data":
        ...
        st.plotly_chart(graficoBacteria(datosBacteriasGlobales[0]), width='stretch')
        st.markdown('### Graph with slider')
        #st.markdown('### World map with slider')
        st.plotly_chart(graficoRS(datosRangeSlider[0]), width='stretch')
        
        left_coll, right_coll=st.columns(2, gap='medium')
        with left_coll:
            st.markdown('### This is the data used to create the world maps')
            st.dataframe(dfConcat[0], width='stretch')
        with right_coll:
            st.markdown('### Bacteria frequency per country')
            tempdfff=pd.read_csv(RAW_DIR / 'geography_data.csv')
            st.dataframe(tempdfff, width='stretch')
    elif seleccion_secundaria == "Bacteria: Abiotrophia":
        st.markdown('### World map')
        st.plotly_chart(graficoBacteria(datosBacteriasGlobales[1]), width='stretch')
        st.markdown('### Graph with slider')
        st.plotly_chart(graficoRS(datosRangeSlider[1]), width='stretch')
        st.markdown('### This is the data used to create the world maps')
        st.dataframe(dfConcat[1], width='stretch')
    elif seleccion_secundaria == "Bacteria: Acidovorax":
        st.markdown('### World map')
        st.plotly_chart(graficoBacteria(datosBacteriasGlobales[2]), width='stretch')
        st.markdown('### Graph with slider')
        st.plotly_chart(graficoRS(datosRangeSlider[2]), width='stretch')
        st.markdown('### This is the data used to create the world maps')
        st.dataframe(dfConcat[2], width='stretch')
    elif seleccion_secundaria == "Bacteria: Cyanibacteria":
        st.markdown('### Gráfico mapa mundi')
        st.plotly_chart(graficoBacteria(datosBacteriasGlobales[3]), width='stretch')
        st.markdown('### Graph with slider')
        st.plotly_chart(graficoRS(datosRangeSlider[3]), width='stretch')
        st.markdown('### This is the data used to create the world maps')
        st.dataframe(dfConcat[3], width='stretch')
    elif seleccion_secundaria == "Bacteria: Megasphaera":
        st.markdown('### Gráfico mapa mundi')
        st.plotly_chart(graficoBacteria(datosBacteriasGlobales[4]), width='stretch')
        st.markdown('### Graph with slider')
        st.plotly_chart(graficoRS(datosRangeSlider[4]), width='stretch')
        st.markdown('### This is the data used to create the world maps')
        st.dataframe(dfConcat[4], width='stretch')
    elif seleccion_secundaria == "Bacteria: Mycobacterium":
        st.markdown('### Gráfico mapa mundi')
        st.plotly_chart(graficoBacteria(datosBacteriasGlobales[5]), width='stretch')
        st.markdown('### Graph with slider')
        st.plotly_chart(graficoRS(datosRangeSlider[5]), width='stretch')
        st.markdown('### This is the data used to create the world maps')
        st.dataframe(dfConcat[5], width='stretch')
    elif seleccion_secundaria == "Bacteria: Neisseria":
        st.markdown('### Gráfico mapa mundi')
        st.plotly_chart(graficoBacteria(datosBacteriasGlobales[6]), width='stretch')
        st.markdown('### Graph with slider')
        st.plotly_chart(graficoRS(datosRangeSlider[6]), width='stretch')
        st.markdown('### This is the data used to create the world maps')
        st.dataframe(dfConcat[6], width='stretch')
    elif seleccion_secundaria == "Bacteria: Prevotella":
        st.markdown('### Gráfico mapa mundi')
        st.plotly_chart(graficoBacteria(datosBacteriasGlobales[7]), width='stretch')
        st.markdown('### Graph with slider')
        st.plotly_chart(graficoRS(datosRangeSlider[7]), width='stretch')
        st.markdown('### This is the data used to create the world maps')
        st.dataframe(dfConcat[7], width='stretch')
    elif seleccion_secundaria == "Bacteria: Streptococcus":
        st.markdown('### Gráfico mapa mundi')
        st.plotly_chart(graficoBacteria(datosBacteriasGlobales[8]), width='stretch')
        st.markdown('### Graph with slider')
        st.plotly_chart(graficoRS(datosRangeSlider[8]), width='stretch')
        st.markdown('### This is the data used to create the world maps')
        st.dataframe(dfConcat[8], width='stretch')
    elif seleccion_secundaria == "Bacteria: Veillonella":
        st.markdown('### Gráfico mapa mundi')
        st.plotly_chart(graficoBacteria(datosBacteriasGlobales[9]), width='stretch')
        st.markdown('### Graph with slider')
        st.plotly_chart(graficoRS(datosRangeSlider[9]), width='stretch')
        st.markdown('### This is the data used to create the world maps')
        st.dataframe(dfConcat[9], width='stretch')
    ...
    st.plotly_chart(graphTaxonomy(), width='stretch')
    ...
    st.markdown('''
                ...
                '''
                , unsafe_allow_html=True)
    st.image('dashboard/img/circosPoster.png')

    st.plotly_chart(graficoHost(), width='stretch')
    st.plotly_chart(treemapgraph(), width='stretch')
    ...
    st.markdown('''
                ...
                ''', unsafe_allow_html=True)
    #st.plotly_chart(treemapgraph(), width='stretch')
    st.plotly_chart(graficoVarGen(), width='stretch')
    ...
    st.markdown('''
                ...
                Acknowledgement: Departamento de Informática y Computación, UTEM; Escuela de Informática, UTEM; Laboratorio de Investigación Aplicada, Departamento de Informática y Computación, UTEM.

                This is the first version of the project and was fully developed by students and professors of Universidad Tecnológica Metropolitana. https://www.utem.cl/universidad/
                ''', unsafe_allow_html=True)
